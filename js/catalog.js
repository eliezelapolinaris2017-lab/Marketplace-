  ...
  try {
    ...
    if (els.productList) {
      els.productList.innerHTML =
        '<div class="empty-state">Error cargando productos. Revisa consola y reglas.</div>';
    }
  }
}
...
function renderTabs(categories) {
  ...
  els.categoryTabs.innerHTML = tabs.map(tab => `
    <button class="tab-btn ${tab.slug === currentCategory ? 'active' : ''}" data-slug="${escapeHtml(tab.slug)}" type="button">
      ${escapeHtml(tab.name)}
    </button>
  `).join('');
    ...
    <article class="product-card">
      ...
      <div>
        ...
        <div class="price-row">
          ...
          <button class="primary-btn js-add" data-id="${escapeAttribute(product.id)}" type="button">
            Comprar
          </button>
        </div>
      </div>
